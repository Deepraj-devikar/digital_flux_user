...
import com.dolphintechno.dolphindigitalflux.adapter.AdapterShop;
import com.dolphintechno.dolphindigitalflux.data.DataGenerator;
...
//                            Toast.makeText(getApplicationContext(), "--"+strUrlAvtarImg, Toast.LENGTH_LONG).show();
    ...
    private void initComponent() {
        recyclerView = (RecyclerView) findViewById(R.id.recyclerView_shop_grid);
        ...
        mAdapter = new AdapterShop(this, items);
        recyclerView.setAdapter(mAdapter);

        // on item list clicked
        mAdapter.setOnItemClickListener(new AdapterShop.OnItemClickListener() {
            @Override
    ...
    public void fetchProductCategories(){
        ...
        recyclerView.addItemDecoration(new SpacingItemDecoration(2, Tools.dpToPx(getApplicationContext(), 8), true));
        recyclerView.setHasFixedSize(true);
        recyclerView.setNestedScrollingEnabled(false);

        shopCategoryList = new ArrayList<>();

        StringRequest stringRequest = new StringRequest(Request.Method.POST, URLs.url_shop_categories,
                new Response.Listener<String>() {
                    ...
                    public void onResponse(String response) {
                        ...
                        try {
                            ...
                            boolean isFoundCategories = jsonObject.getBoolean(URLs.keyIsFoundCategories);
                            if (isFoundCategories){
                                JSONArray jsonList = jsonObject.getJSONArray(URLs.keyCategoriesList);
                                for (int i = 0; i < jsonList.length(); i++){
                                    JSONObject dataObject = jsonList.getJSONObject(i);
                                    shopCategory = new ShopCategory();
                                    shopCategory.categoryId = dataObject.getString(URLs.keyCategoriesId);
                                    shopCategory.title = dataObject.getString(URLs.keyCategoriesName);
                                    shopCategory.ctegoryImage = dataObject.getString(URLs.keyCategoriesImage);
                                    shopCategoryList.add(shopCategory);
                                }
                                //set data and list adapter
                                mAdapter = new AdapterShop(getApplicationContext(), shopCategoryList);
                                recyclerView.setAdapter(mAdapter);
                                ...
                                mAdapter.setOnItemClickListener(new AdapterShop.OnItemClickListener() {
                                    @Override
                                    public void onItemClick(View view, ShopCategory obj, int position) {
//                                        Intent intent = new Intent(getApplicationContext(), ShopSubCategory.class);
//                                        intent.putExtra(IntentKeys.keyShopCategoryId, obj.categoryId);
//                                        intent.putExtra(IntentKeys.keyShopCategoryName, obj.title);
...
//                                        Snackbar.make(parent_view, "Item " + obj.title + " clicked", Snackbar.LENGTH_SHORT).show();
            ...
            protected Map<String, String> getParams() throws AuthFailureError {
                ...
                params.put(URLs.keyCategoriesCode, URLs.valueCatrgoriesCode);
                return params;
            }
        };
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        this.menu = menu;
        getMenuInflater().inflate(R.menu.menu_cart_setting, this.menu);
        badgeMenuItem = this.menu.findItem(R.id.action_cart);
